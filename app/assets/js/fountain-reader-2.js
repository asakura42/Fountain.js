      ...
      return this.each(function () {
        ...
        $app.on('fountain-js:loaded', function (e, file) {
          ...
          fountain.parse(file, true, function (result) {
            ...
            var character_stats = []
            var last_character = ''
        /*    $(output.tokens).each(function(i){

              if($(this)[0].type === 'character') {
              	last_character = output.tokens[i].text
              	if( character_stats[last_character] == undefined ) character_stats[last_character] = 0
              }

              //Determine if we are looking at a scene heading
              if($(this)[0].type === 'dialogue') {

                this.character = last_character
                character_stats[this.character] += 1

             }});*/
         $('.dialogue').each(function(){
         	let character = $(this).children('h4').text()
